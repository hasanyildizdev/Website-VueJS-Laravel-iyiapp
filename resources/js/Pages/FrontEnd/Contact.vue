...
<template>
    <section id="contact" class="min-h-screen py-14 bg-gradient-to-b via-black from-black to-my_purple3">
        <div class="w-full flex flex-col px-6 lg:px-40">
            <div class=" flex items-center lg:justify-start justify-center">
                <h2 class="text-3xl lg:text-5xl text-white font-tech"> {{ $t('contact') }} </h2>
            </div>
            <div class="flex flex-col lg:flex-row items-center justify-between py-10">
                <div class="text-white lg:mr-16 lg:min-w-[400px] min-w-[250px]">
                    <div class="w-full ml-4 mb-6">
                        <h4 class="text-white font-roboto lg:text-2xl"> {{ $t('buildidea') }} </h4>
                    </div>
                    <div class="mb-8 flex w-full">
                        ...
                        <div class="w-full">
                            <h4 class="text-dark mb-1 text-xl font-bold"> {{ $t('location') }} </h4>
                    ...
                    <div class="mb-8 flex w-full max-w-[370px]">
                        ...
                        <div class="w-full">
                            <h4 class="text-dark mb-1 text-xl font-bold"> {{ $t('phone') }} </h4>
                            <p class="text-body-color text-base">(+90)546 933 2197</p>
                        </div>
                    </div>
                    <div class="mb-8 flex w-full max-w-[370px]">
                        <div
                        ...
                        <div class="w-full">
                            <h4 class="text-dark mb-1 text-xl font-bold"> {{ $t('email') }} </h4>
                            <p class="text-body-color text-base">info@iyiapp.com</p>
                        </div>
                    </div>
                </div>
                <div class=" border-2 border-my_purple rounded-2xl lg:px-10 py-4">
                    <div class="flex items-center justify-center lg:mb-2">
                        <p class="text-my_purple text-3xl lg:text-4xl font-lale"> {{ $t('sendmail') }} </p>
                    </div>
                    <form @submit.prevent="submit" class="p-4 lg:p-0 space-y-2 lg:space-y-4 max-w-md">
                        <div v-if="showMessage" class="p-4 bg-my_purple2 text-white rounded-lg">
                            {{ $t('thankyou') }}
                        </div>
                        <div class="flex gap-2 lg:gap-4 ">
                            <div>
                                <input v-model="form.name" type="text" class="input " :placeholder="$t('yourname')" />
                                <span v-if="form.errors.name" class="text-sm m-2 text-red">{{
                                    form.errors.name
                                }}</span>
                            </div>
                            <div>
                                <input v-model="form.email" type="email" class="input" :placeholder="$t('youremail')" />
                                <span v-if="form.errors.email" class="text-sm m-2 text-red">{{
                                    form.errors.email
                                }}</span>
                            </div>
                        </div>
                        <textarea v-model="form.body" class="textarea" :placeholder="$t('yourmessage')"
                            spellcheck="false"></textarea>
                        <span v-if="form.errors.body" class="text-sm m-2 text-red">{{
                            form.errors.body
                        }}</span>
                        <div class="w-full lg:pb-2 flex items-center justify-center ">
                            <button class="myButton2 px-12">
                                {{ $t('sendButton') }}
                            </button>
                        </div>
                    </form>
